...
pub trait NvgreOrchCallbacks: Send + Sync {
    ...
    fn create_tunnel(
        &self,
        src_ip: &IpAddress,
        tunnel_ids: &TunnelSaiIds,
        underlay_rif: RawSaiObjectId,
    ) -> Result<RawSaiObjectId, String>;
    ...
    fn create_tunnel_map_entry(
        ...
        vlan_id: u16,
        is_encap: bool,
        encap_map_id: RawSaiObjectId,
        decap_map_id: RawSaiObjectId,
    ) -> Result<RawSaiObjectId, String>;
...
impl NvgreOrch {
    ...
    pub fn create_tunnel(&mut self, config: NvgreTunnelConfig) -> Result<(), NvgreOrchError> {
        ...
        for map_type in &[MapType::Vlan, MapType::Bridge] {
            ...
            if let (Ok(encap_id), Ok(decap_id)) = (
                ...
                tunnel
                    .tunnel_ids
                    .tunnel_encap_id
                    .insert(*map_type, encap_id);
                tunnel
                    .tunnel_ids
                    .tunnel_decap_id
                    .insert(*map_type, decap_id);
            }
        }

        // Create tunnel
        let underlay_rif = callbacks.get_underlay_rif();
        tunnel.tunnel_ids.tunnel_id =
            match callbacks.create_tunnel(&config.src_ip, &tunnel.tunnel_ids, underlay_rif) {
                Ok(id) => id,
                Err(e) => {
                    let err = NvgreOrchError::SaiError(e);
                    audit_log!(AuditRecord::new(
        ...
        tunnel.tunnel_ids.tunnel_term_id = match callbacks.create_tunnel_termination(
            tunnel.tunnel_ids.tunnel_id,
            &config.src_ip,
            vr_id,
        ) {
    ...
    pub fn remove_tunnel_map(
        &mut self,
        tunnel_name: &str,
        map_entry_name: &str,
    ) -> Result<(), NvgreOrchError> {
...
mod tests {
    ...
    impl NvgreOrchCallbacks for MockCallbacks {
        ...
        fn create_tunnel_map_entry(
            ...
            _vlan_id: u16,
            _is_encap: bool,
            _encap_map_id: RawSaiObjectId,
            _decap_map_id: RawSaiObjectId,
        ) -> Result<RawSaiObjectId, String> {
    ...
    fn test_remove_tunnel_with_map_entries() {
        ...
        let map_config2 =
            NvgreTunnelMapConfig::new("tunnel1".to_string(), "map2".to_string(), 200, 2000);
    ...
    fn test_tunnel_configuration_with_different_ips() {
        ...
        assert_eq!(
            orch.get_tunnel("tunnel1").unwrap().src_ip,
            test_ip_v4(192, 168, 1, 1)
        );
        assert_eq!(
            orch.get_tunnel("tunnel2").unwrap().src_ip,
            test_ip_v4(172, 16, 0, 1)
        );
    }
    ...
    fn test_add_tunnel_map_tunnel_not_found() {
        let mut orch = create_test_orch();
        let map_config =
            NvgreTunnelMapConfig::new("nonexistent".to_string(), "map1".to_string(), 100, 1000);
        let result = orch.add_tunnel_map(map_config);
        assert!(matches!(result, Err(NvgreOrchError::TunnelNotFound(_))));
    }
    ...
    fn test_vsid_to_vlan_mapping() {
        ...
        let map_config2 =
            NvgreTunnelMapConfig::new("tunnel1".to_string(), "map2".to_string(), 200, 2000);
        let map_config3 =
            NvgreTunnelMapConfig::new("tunnel1".to_string(), "map3".to_string(), 300, 3000);
    ...
    fn test_vsid_range_validation_min_valid() {
        ...
        let map_config =
            NvgreTunnelMapConfig::new("tunnel1".to_string(), "map1".to_string(), 100, 1);
        let result = orch.add_tunnel_map(map_config);
        assert!(result.is_ok());
    }
    ...
    fn test_multiple_mappings_per_tunnel() {
        ...
        // Remove half of them
        for i in 0..5 {
            orch.remove_tunnel_map("tunnel1", &format!("map{}", i))
                .unwrap();
        }
    ...
    fn test_vlan_not_found() {
        ...
        let map_config =
            NvgreTunnelMapConfig::new("tunnel1".to_string(), "map1".to_string(), 999, 1000);
        let result = orch.add_tunnel_map(map_config);
    ...
    fn test_statistics_tracking() {
        ...
        let map_config1 =
            NvgreTunnelMapConfig::new("tunnel1".to_string(), "map1".to_string(), 100, 1000);
        let map_config2 =
            NvgreTunnelMapConfig::new("tunnel2".to_string(), "map2".to_string(), 200, 2000);
        orch.add_tunnel_map(map_config1).unwrap();
        orch.add_tunnel_map(map_config2).unwrap();
    ...
    fn test_tunnel_count_accuracy() {
        ...
        for i in 0..5 {
            let config =
                NvgreTunnelConfig::new(format!("tunnel{}", i), test_ip_v4(10, 0, 0, i as u8 + 1));
            orch.create_tunnel(config).unwrap();
        }
    ...
    fn test_map_entry_names_case_sensitive() {
        ...
        let map_config1 =
            NvgreTunnelMapConfig::new("tunnel1".to_string(), "Map1".to_string(), 100, 1000);
        let map_config2 =
            NvgreTunnelMapConfig::new("tunnel1".to_string(), "map1".to_string(), 200, 2000);
    ...
    fn test_complex_scenario_multiple_tunnels_and_maps() {
        ...
        for i in 1..=3 {
            let config =
                NvgreTunnelConfig::new(format!("tunnel{}", i), test_ip_v4(10, 0, 0, i as u8));
            orch.create_tunnel(config).unwrap();
        }
